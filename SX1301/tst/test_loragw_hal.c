...
int HalTestMain(void)
{
	xd = 475.1;/* <float> Radio A RX frequency in MHz */
	fa = (uint32_t)((xd*1e6) + 0.5); /* .5 Hz offset to get rounding instead of truncating */
	xd = 485.1;/* <float> Radio B RX frequency in MHz */
	fb = (uint32_t)((xd*1e6) + 0.5); /* .5 Hz offset to get rounding instead of truncating */
	xd = 495.1;/* <float> Radio TX frequency in MHz */
        ...
        else 
        {
            ...
            for(i=0; i < nb_pkt; ++i)
            {
                ...
                if (p->status == STAT_CRC_OK) 
                {
                    ...
                    switch (p->coderate) 
                    {
                        case CR_LORA_4_5: printf(" CR1(4/5)"); 
											break;
                        case CR_LORA_4_6: printf(" CR2(2/3)"); 
											break;
                        case CR_LORA_4_7: printf(" CR3(4/7)");
											break;
                        case CR_LORA_4_8: printf(" CR4(1/2)"); 
											break;
