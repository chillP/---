...
//    int option_index = 0;
    ...
    while ((i = getopt_long (argc, argv, "hif:n:m:b:s:c:p:l:z:t:x:r:k:d:q:", long_options, &option_index)) != -1) {
		...
		//test wyk 20160103
		/* <float> Target frequency in MHz */
		xd = 475.3;
		f_target = (uint32_t)((xd*1e6) + 0.5); /* .5 Hz offset to get rounding instead of truncating */
		
		/* <uint> TX notch filter frequency in kHz */
		xi = 200;
		tx_notch_freq = xi*1000;
		
		/* <str> Modulation type */
		
		/* <int> Modulation bandwidth in kHz */
		 xi = 125;
		i = 1;
			if ((i != 1) || ((xi != 125) && (xi != 250) && (xi != 500))) 
			{
			...
			/* <int> numbers of times the sequence is repeated */
			xi = 2;
			if ((i != 1) || (xi < -1)) 
			{
			MSG("ERROR: invalid number of repeats\n");
			...
			/* Send packet using inverted modulation polarity */
			invert = true;


			/* <int> Concentrator clock source (Radio A or Radio B) */
			xi = 1;
			if ((i != 1) || ((xi != 0) && (xi != 1))) 
			{
			MSG("ERROR: invalid clock source\n");
      ...
      #if 0
			if( strcmp(long_options[option_index].name, "lbt-freq") == 0 ) 
			{ /* <float> LBT first channel frequency in MHz */
				i = sscanf(optarg, "%lf", &xd);
				if ((i != 1) || (xd < 30.0) || (xd > 3000.0)) 
				{
					MSG("ERROR: invalid LBT start frequency\n");
			...
			else if( strcmp(long_options[option_index].name, "lbt-sctm") == 0 ) 
			{ /* <int> LBT scan time in usec */
				if (lbt_enable == true) 
				{
					i = sscanf(optarg, "%i", &xi);
					if ((i != 1) || (xi < 0)) 
			...
			else if( strcmp(long_options[option_index].name, "lbt-rssi") == 0 ) 
			{ /* <int> LBT RSSI target */
				if (lbt_enable == true) 
				{
					i = sscanf(optarg, "%i", &xi);
			...
			else if( strcmp(long_options[option_index].name, "lbt-rssi-offset") == 0 )
			{ /* <int> LBT RSSI offset */
				if (lbt_enable == true) 
				{
					i = sscanf(optarg, "%i", &xi);
			...
			else if( strcmp(long_options[option_index].name, "lbt-nbch") == 0 ) 
			{ /* <int> LBT number of channels */
				if (lbt_enable == true) 
				{
					i = sscanf(optarg, "%i", &xi);
					if ((i != 1) || (xi < 0)) 
