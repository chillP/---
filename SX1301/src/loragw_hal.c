...
uint8_t fw_check[MCU_AGC_FW_BYTE];
...
int load_firmware(uint8_t target, const uint8_t *firmware, uint16_t size);
...
int load_firmware(uint8_t target,	const uint8_t *firmware, uint16_t size) {
    ...
    //lgw_reg_wb_const(LGW_MCU_PROM_DATA, firmware, size);		
    lgw_reg_wb(LGW_MCU_PROM_DATA, (uint8_t *)(firmware), size);
	...
	for(uint16_t i = 0;i<size;i++)
	{
		...
		chushu = size/quantity;//打印进度
		for(uint16_t i = 0;i<size;i++)
		{
			if(fw_check[i]!=firmware[i])
			{
				printf ("ERROR: Failed to load fw %d\n", (int)target);
				printf ("ERROR: load firmware compare error %d\n", i);
				return -1;
			}
			if(i%chushu == 0)//打印进度
			{
...
void lgw_constant_adjust(void) {
			...
			int32_t temp;
	/* I/Q path setup */
		//wyk 2017.03.21 19.34
    // lgw_reg_w(LGW_RX_INVERT_IQ,0); /* default 0 */
	//lgw_reg_w(LGW_RX_INVERT_IQ,1); /* default 0 */
	#if 1
			{
					if(SystemParameter.ReceiveIQInvert)
				{
					lgw_reg_w(LGW_RX_INVERT_IQ,1); /* default 0 */
					//lgw_reg_w(LGW_MBWSSF_MODEM_INVERT_IQ,1); /* default 0 */
				}
				else
				{
					lgw_reg_w(LGW_RX_INVERT_IQ,0); /* default 0 */
				...
				lgw_reg_r( LGW_RX_INVERT_IQ, &temp ); /* bug workaround */
				printf("\ninvert %d\n",temp);
			}
			
			#endif
	//end 2017.03.21 19.34
    ...
    // lgw_reg_w(LGW_PREAMBLE_SYMB1_NB,10); /* default 10 */
		lgw_reg_w(LGW_PREAMBLE_SYMB1_NB,SystemParameter.RxPreambleLen); /* default 10 */
		//end 2017.05.09
    ...
    //wyk 2017.03.21
    //lgw_reg_w(LGW_ONLY_CRC_EN,1); /* default 1 */ //wyk 2017.03.17 lgw_reg_w(LGW_ONLY_CRC_EN,1)
    if(SystemParameter.ReceiveCRC & REC_NO_CRC)
    {
     	lgw_reg_w(LGW_ONLY_CRC_EN,0); //同时接收
    }
    else
    {
			lgw_reg_w(LGW_ONLY_CRC_EN,1); 
    }
    //end 2017.03.21
        ...
        case IF_LORA_STD:
            ...
            //wyk 2017.03.20兼容银通
                ...
                lora_rx_ppm_offset = false;
						...
						#else
						if ((SET_PPM_ON(conf.bandwidth,conf.datarate))||((conf.bandwidth == BW_125KHZ)&&(SystemParameter.LowDatarateOptmize &(conf.datarate>1))))
						{
							lora_rx_ppm_offset = true;
						}
						else
...
int lgw_start(void) {
    ...
    lgw_reg_w(LGW_CLK32M_EN, 0);
		...
		read_val = 1;
		lgw_reg_r(LGW_GLOBAL_EN, &read_val);
    read_val = 2;
    lgw_reg_r(LGW_CLK32M_EN, &read_val);
	...
	lgw_reg_w(LGW_GLOBAL_EN, 1);
		    ...
		    /* Enable clocks *///2016 12 30 wangyk
    //lgw_reg_w(LGW_GLOBAL_EN, 1);
    ...
    lgw_reg_w(LGW_RADIO_B_EN,1);
		...
		read_val = 2;
		lgw_reg_r(LGW_RADIO_A_EN, &read_val);
    read_val = 2;
    lgw_reg_r(LGW_RADIO_B_EN, &read_val);
		
    //wait_ms(500); /* TODO: optimize */
		WaiteTimeDisplay(500);
		//wait_ms(1000); /* TODO: optimize */
    ...
    lgw_reg_w(LGW_GPIO_MODE,31); /* Set all GPIOs as output */
		...
		testvalue = 0;
		lgw_reg_r(LGW_GPIO_MODE,&testvalue); /* Set all GPIOs as output */
		printf("if lgw_gpio_mode set ok, actual:%d expected:31\n", testvalue);
		
    //lgw_reg_w(LGW_GPIO_SELECT_OUTPUT,2);//驱动原始代码
		lgw_reg_w(LGW_GPIO_SELECT_OUTPUT,0);//改为GPIO0->rxbuff_not_empty gpio4->tx_on wyk
		
		testvalue = 0;
		lgw_reg_r(LGW_GPIO_SELECT_OUTPUT,&testvalue); /* Set all GPIOs as output */
		printf("if lgw_gpio_select set ok, actual:%d expected:31\n", testvalue);
    ...
    */
    ...
    //cal_time = 2300; /* measured between 2.1 and 2.2 sec, because 1 TX only */
		cal_time = 2300; /* measured between 2.1 and 2.2 sec, because 1 TX only */ //原来是2300先试下
    ...
    lgw_reg_w(LGW_MCU_RST_1, 0);
		...
		read_val = 2;
		lgw_reg_r(LGW_FORCE_HOST_RADIO_CTRL, &read_val);
    read_val = 2;
    lgw_reg_r(LGW_RADIO_SELECT, &read_val);
		read_val = 2;
    lgw_reg_r(LGW_MCU_RST_1, &read_val);

#if 1		
    /* Check firmware version */
    lgw_reg_w(LGW_DBG_AGC_MCU_RAM_ADDR, FW_VERSION_ADDR);
		//先赋值再查回来2016 12 30 wangyk
		read_val = 0;
		//end 2016 12 30
	...
	{
		printf("SUCCESS: Version of calibration firmware expected, actual:%d expected:%d\n", fw_version, FW_VERSION_CAL);
	}
#endif
    ...
    lgw_reg_w(LGW_EMERGENCY_FORCE_HOST_CTRL, 0); /* Give control of concentrator registers to MCU */
		...
		read_val = 2;
		lgw_reg_r(LGW_PAGE_REG, &read_val);
    read_val = 2;
    lgw_reg_r(LGW_EMERGENCY_FORCE_HOST_CTRL, &read_val);

    /* Wait for calibration to end */
    //DEBUG_PRINTF("Note: calibration started (time: %u ms)\n", cal_time);
		//DEBUG_PRINTF("Note: calibration started \n");//wyk 2017.03.19
		printf("Note: calibration started \n");
    //wait_ms(cal_time); /* Wait for end of calibration */
    WaiteTimeDisplay(cal_time);
    ...
    */
    ...
    if (rf_enable[1] && rf_tx_enable[1] && ((cal_status & 0x40) == 0)) {
		...
		//2017.05.19 保证启动成功,失败返回
		#ifndef DebugCalibrationSkip
		if((rf_enable[0] && ((cal_status & 0x02) == 0))||(rf_enable[1] && ((cal_status & 0x04) == 0))||
			(rf_enable[0] && ((cal_status & 0x08) == 0))||(rf_enable[1] && ((cal_status & 0x10) == 0))||
			(rf_enable[0] && rf_tx_enable[0] && ((cal_status & 0x20) == 0)) ||
			(rf_enable[1] && rf_tx_enable[1] && ((cal_status & 0x40) == 0))
			)
		{
    ...
    */
    ...
    lgw_reg_w(LGW_CORR7_DETECT_EN, (if_enable[7] == true) ? lora_multi_sfmask[7] : 0); /* default 0 */
		...
		//wyk 为了兼容银通已经使用的表 2017.03.17
    //lgw_reg_w(LGW_PPM_OFFSET, 0x60); /* as the threshold is 16ms, use 0x60 to enable ppm_offset for SF12 and SF11 @125kHz*/   
		lgw_reg_w(LGW_PPM_OFFSET, SystemParameter.LowDatarateOptmize); /* as the threshold is 16ms, use 0x60 to enable ppm_offset for SF12 and SF11 @125kHz*/   
        ...
        if ((ifmod == IF_LORA_MULTI) || (ifmod == IF_LORA_STD)) {
            ...
            if (SET_PPM_ON(p->bandwidth,p->datarate)) {
                ...
                ppm = 0;
						...
						#endif						
						if ((SET_PPM_ON(p->bandwidth,p->datarate))||((p->bandwidth == BW_125KHZ)&&(SystemParameter.LowDatarateOptmize &(p->datarate>1))))
						{
                ppm = 1;
            } 
						else 
    ...
    if (pkt_data.modulation == MOD_LORA) {
        ...
        if (SET_PPM_ON(pkt_data.bandwidth,pkt_data.datarate)) 
            ...
            buff[11] |= 0x08; /* set 'PPM offset' bit at 1 */
				...
				#endif
				if((SET_PPM_ON(pkt_data.bandwidth,pkt_data.datarate))||((pkt_data.bandwidth == BW_125KHZ)&&(SystemParameter.LowDatarateOptmize &(pkt_data.datarate>1))))
				{
					buff[11] |= 0x08; /* set 'PPM offset' bit at 1 */
				}
				// end 03.20
				
				//wyk 2017.03.21 19.31						
...
uint32_t lgw_time_on_air(struct lgw_pkt_tx_s *packet) {
    ...
    if (packet->modulation == MOD_LORA) {
        ...
        /* Duration of packet */
        //Tpacket = Tpreamble + Tpayload;//ms
				Tpacket = (Tpreamble + Tpayload)*1000;//us
